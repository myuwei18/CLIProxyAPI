...
function quotaModule(title,free,total){const t=Number(total||0);const f=Number(free||0);const percent=t>0?Math.max(0,Math.min(100,Math.round(f*100/t))):(f>0?100:0);const cls=percent>=50?'ok':(percent>=20?'warn':'err');return '<div class="quota-module"><div class="quota-module-head"><div class="quota-module-title">'+title+'</div><div class="quota-module-amount">'+money(f)+' / '+money(t)+'</div></div><div class="bar-row"><div class="bar"><div class="fill '+cls+'" style="width:'+percent+'%"></div></div><div class="bar-meta">'+percent+'%</div></div></div>'}
