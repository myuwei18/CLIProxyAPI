...
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif}main{max-width:1320px;margin:0 auto;padding:40px 18px 32px}.page-head{margin-bottom:12px}.page-kicker{font-size:12px;color:var(--muted);font-weight:600;margin-bottom:4px}h1{margin:0 0 4px;font-size:22px;font-weight:700;letter-spacing:-.02em}h2{margin:0;font-size:16px;font-weight:700}p{margin:0;color:var(--muted);line-height:1.5}.sub{font-size:12px}.grid{display:grid;grid-template-columns:repeat(12,1fr);gap:10px}.card{background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:12px;box-shadow:var(--shadow)}.span-2{grid-column:span 2}.span-3{grid-column:span 3}.span-4{grid-column:span 4}.span-8{grid-column:span 8}.span-12{grid-column:span 12}.tabs,.authbar,.toolbar,.actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.toolbar{justify-content:space-between;margin-bottom:10px}.toolbar-left{display:flex;align-items:center;gap:10px;flex-wrap:wrap}button,input,textarea{font:inherit}button{border:1px solid var(--blue);border-radius:6px;padding:6px 10px;background:var(--blue);color:#fff;cursor:pointer;font-weight:600;font-size:13px;line-height:1.2}button.secondary{background:var(--panel);color:var(--text);border:1px solid var(--line)}button.tab,button.auth-tab{background:var(--panel);color:var(--muted);border:1px solid var(--line);box-shadow:none;font-weight:600}button.tab.active,button.auth-tab.active{background:var(--panel2);color:var(--text);border-color:var(--blue)}input,textarea{width:100%;border:1px solid var(--line);border-radius:6px;background:var(--panel2);color:var(--text);padding:7px 9px}label{display:block;color:var(--muted);font-size:12px;font-weight:600;margin:0 0 4px}.metric{min-height:78px}.metric .label{color:var(--muted);font-size:12px}.metric .value{font-size:22px;font-weight:750;margin-top:5px}.metric .hint{color:var(--muted);font-size:12px;margin-top:4px}.muted{color:var(--muted)}.small{font-size:12px}.status-dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--muted);margin-right:6px}.status-dot.ok{background:var(--green)}.status-dot.warn{background:var(--amber)}.status-dot.err{background:var(--red)}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:8px}table{width:100%;border-collapse:separate;border-spacing:0}th,td{text-align:left;padding:7px 8px;border-bottom:1px solid var(--line);vertical-align:top;font-size:12px}th{color:var(--muted);font-size:11px;background:var(--panel2);font-weight:700}tr:last-child td{border-bottom:0}.quota-cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:10px}.quota-card{border:1px solid var(--line);border-radius:8px;background:var(--panel);padding:12px}.quota-card.unavailable{opacity:.68;background:var(--panel2)}.quota-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:10px}.quota-email{font-weight:700;font-size:14px}.quota-meta{color:var(--muted);font-size:12px;margin-top:3px}.quota-status{font-size:12px;color:var(--muted);white-space:nowrap}.quota-status.ok{color:var(--green)}.quota-status.warn{color:var(--amber)}.quota-status.err{color:var(--red)}.quota-times{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:6px}.quota-time-right{text-align:right}.quota-main{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px}.quota-module{border:1px solid var(--line);border-radius:8px;padding:8px;background:var(--panel2)}.quota-module-head{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px}.quota-module-title{font-size:12px;font-weight:650;color:var(--text)}.quota-module-amount{font-size:12px;color:var(--muted);white-space:nowrap}.quota-module-reset{font-size:11px;color:var(--muted);margin-top:6px}.bar-row{display:grid;grid-template-columns:1fr 42px;align-items:center;gap:8px;margin-top:4px}.bar-title{font-weight:650;color:var(--muted);font-size:12px}.bar{height:7px;background:var(--track);border-radius:999px;overflow:hidden}.fill{height:100%;border-radius:999px}.fill.ok{background:var(--green)}.fill.warn{background:var(--amber)}.fill.err{background:var(--red)}.bar-meta{text-align:right;font-size:12px;color:var(--muted)}.errorbox{display:none;border:1px solid rgba(220,38,38,.35);background:rgba(220,38,38,.08);color:var(--red);border-radius:8px;padding:9px 11px;margin-bottom:10px;white-space:pre-wrap}.statusbox{display:none;border:1px solid var(--line);background:var(--panel2);color:var(--text);border-radius:8px;padding:8px 10px;margin:8px 0;font-size:12px}.statusbox.ok{border-color:rgba(22,163,74,.35);color:var(--green)}.statusbox.err{border-color:rgba(220,38,38,.35);color:var(--red)}.notice{border:1px solid var(--line);background:var(--panel2);border-radius:8px;padding:9px;line-height:1.45}.form-row{margin-bottom:8px}.forms{display:grid;grid-template-columns:1fr 1fr;gap:10px}.auth-panel{display:none;margin-top:10px}.auth-panel.active{display:block}textarea{min-height:96px;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;font-size:11px}code{background:var(--panel2);border:1px solid var(--line);border-radius:6px;padding:1px 5px}.authbar{margin-bottom:10px}.authbar .field{width:190px}@media(max-width:980px){main{padding-top:24px}.span-2,.span-3,.span-4,.span-8{grid-column:span 12}.forms{grid-template-columns:1fr}.quota-main,.quota-times{grid-template-columns:1fr}.quota-time-right{text-align:left}}
</style>
</head>
<body>
<main>
<div class="page-head"><div class="page-kicker">CPA Plugin</div><h1>FreeModel 供应商运营面板</h1><p class="sub">脱敏运营视图 · executor 默认关闭 · 跟随管理后台/系统主题</p></div>
...
function updateTabs(summary){document.querySelector('[data-view=real]').textContent='全部('+(summary.real_accounts||0)+')';document.querySelector('[data-view=available]').textContent='可用('+(summary.available||0)+')';document.querySelector('[data-view=other]').textContent='不可用('+(summary.other||0)+')'}
function fmtUnix(v){const n=Number(v||0);return n>0?fmtTime(new Date(n*1000).toISOString()):'--'}function quotaModule(title,free,total,resetAt){const t=Number(total||0);const f=Number(free||0);const percent=t>0?Math.max(0,Math.min(100,Math.round(f*100/t))):(f>0?100:0);const cls=percent>=50?'ok':(percent>=20?'warn':'err');return '<div class="quota-module"><div class="quota-module-head"><div class="quota-module-title">'+title+'</div><div class="quota-module-amount">'+money(f)+' / '+money(t)+'</div></div><div class="bar-row"><div class="bar"><div class="fill '+cls+'" style="width:'+percent+'%"></div></div><div class="bar-meta">'+percent+'%</div></div><div class="quota-module-reset">刷新 '+fmtUnix(resetAt)+'</div></div>'}
...
document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));b.classList.add('active');currentView=b.dataset.view;load()});document.getElementById('backBtn').onclick=()=>{try{window.top.location.href='/management.html#/plugins'}catch{window.location.href='/management.html#/plugins'}};document.querySelectorAll('.auth-tab').forEach(b=>b.onclick=()=>{document.querySelectorAll('.auth-tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.auth-panel').forEach(x=>x.classList.remove('active'));b.classList.add('active');document.getElementById(b.dataset.panel).classList.add('active')});document.getElementById('refreshBtn').onclick=async()=>{clearStatus();await load();showStatus('额度已刷新。',true)};document.getElementById('syncBtn').onclick=async()=>{clearError();clearStatus();try{showStatus('正在同步 CK 额度...',true);await postJSON('/sync',{});await load();showStatus('同步完成，额度已更新。',true)}catch(e){showError(e);showStatus('同步失败，请查看错误信息。',false)}};document.getElementById('sendOtpBtn').onclick=async()=>{clearError();clearStatus();const fd=new FormData(document.getElementById('otpForm'));try{showStatus('正在发送验证码...',true);await postJSON('/auth/send-otp',{email:fd.get('email'),proxy:fd.get('proxy')});showStatus('验证码已发送，请查收邮箱。',true)}catch(e){showError(e);showStatus('验证码发送失败。',false)}};document.getElementById('otpForm').onsubmit=async ev=>{ev.preventDefault();clearError();const fd=new FormData(ev.currentTarget);try{await postJSON('/auth/verify-otp',{email:fd.get('email'),code:fd.get('code'),proxy:fd.get('proxy')});ev.currentTarget.querySelector('[name=code]').value='';await load();showStatus('保存成功，账号列表已刷新。',true)}catch(e){showError(e)}};document.getElementById('accountForm').onsubmit=async ev=>{ev.preventDefault();clearError();try{await postJSON('/accounts',Object.fromEntries(new FormData(ev.currentTarget).entries()));ev.currentTarget.reset();await load();showStatus('保存成功，账号列表已刷新。',true)}catch(e){showError(e)}};document.getElementById('importForm').onsubmit=async ev=>{ev.preventDefault();clearError();try{const out=await postJSON('/import/accounts',JSON.parse(new FormData(ev.currentTarget).get('payload')));alert(JSON.stringify(out.summary||out,null,2));await load()}catch(e){showError(e)}};load();
</script>
</body>
</html>`
}
