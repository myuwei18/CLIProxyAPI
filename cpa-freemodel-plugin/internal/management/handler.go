...
function renderAccounts(items){document.getElementById('accountCount').textContent=items.length;document.getElementById('accountRows').innerHTML=items.length?items.map(a=>'<tr><td>'+a.id+'</td><td>'+esc(a.email)+'<br><span class="muted small">'+esc((a.email_hash||'').slice(0,8))+'</span></td><td>'+(a.model_api_configured?'有':'无')+'</td><td>'+esc(a.proxy||'直连')+'<br><button class="secondary account-proxy-edit" data-id="'+a.id+'" type="button">编辑代理</button></td></tr>').join(''):'<tr><td colspan="4" class="muted">暂无账号</td></tr>';document.querySelectorAll('.account-proxy-edit').forEach(b=>b.onclick=async ev=>{const id=ev.currentTarget.dataset.id;const proxy=prompt('填写代理 URL，留空为直连；也可填已保存代理的完整 URL');if(proxy===null)return;try{await fetch(api+'/accounts/proxy',{method:'PATCH',credentials:'same-origin',headers:managementHeaders({'Content-Type':'application/json'}),body:JSON.stringify({account_id:Number(id),proxy})}).then(async r=>{const d=await r.json().catch(()=>({}));if(!r.ok||d.success===false)throw new Error(d.message||('HTTP '+r.status))});await load();showStatus('账号代理已更新。',true)}catch(e){showError(e);showStatus('账号代理更新失败。',false)}})}
function updateTabs(summary){document.querySelector('[data-view=real]').textContent='全部('+(summary.real_accounts||0)+')';document.querySelector('[data-view=available]').textContent='可用('+(summary.available||0)+')';document.querySelector('[data-view=other]').textContent='其他('+(summary.other||0)+')'}
function fmtUnixReset(v){let n=Number(v||0);if(!n)return'--刷新';if(n>100000000000)n=Math.floor(n/1000);return fmtTime(new Date(n*1000).toISOString())+'刷新'}function quotaModule(title,free,total,resetAt){const t=Number(total||0);const f=Number(free||0);const percent=t>0?Math.max(0,Math.min(100,Math.round(f*100/t))):(f>0?100:0);const cls=percent>=50?'ok':(percent>=20?'warn':'err');const label=resetAt?title+' '+fmtUnixReset(resetAt):title;return '<div class="quota-module"><div class="quota-module-head"><div class="quota-module-title">'+label+'</div><div class="quota-module-amount">'+money(f)+' / '+money(t)+'</div></div><div class="bar-row"><div class="bar"><div class="fill '+cls+'" style="width:'+percent+'%"></div></div><div class="bar-meta">'+percent+'%</div></div></div>'}
...
function renderQuota(resp){latestSummary=resp.summary||{};latestQuota=resp.data||[];updateTabs(latestSummary);document.getElementById('realAvailable').textContent=money(latestSummary.real_available_cents);document.getElementById('quotaHint').textContent='窗口 '+money(latestSummary.window_available_cents)+' · 其他 '+money(latestSummary.extra_available_cents);document.getElementById('otherCount').textContent=text(latestSummary.other);document.getElementById('updatedAt').textContent=fmtTime(new Date().toISOString());const rows=latestQuota.filter(q=>!q.is_test_account);document.getElementById('quotaCards').innerHTML=rows.length?rows.map(q=>{const st=quotaState(q);const unavailable=st.cls==='ok'?'':' unavailable';const extra=Number(q.extra_available_cents||0);return '<div class="quota-card'+unavailable+'"><div class="quota-head"><div style="min-width:0;flex:1"><div class="quota-email">#'+esc(q.account_id)+' '+esc(q.email)+'</div></div><div class="quota-status '+st.cls+'"><span class="status-dot '+st.cls+'"></span>'+st.label+' · '+money(q.real_available_cents)+'</div></div><div class="quota-times"><div class="quota-meta">有效期 '+fmtTime(q.current_period_end)+'</div><div class="quota-meta quota-time-right">更新时间 '+fmtTime(q.fetched_at)+'</div></div><div class="quota-main">'+quotaModule('5h',q.window_5h_remaining,q.window_5h&&q.window_5h.limit_cents,q.window_5h&&q.window_5h.resets_at)+quotaModule('7d',q.window_week_remaining,q.window_week&&q.window_week.limit_cents,q.window_week&&q.window_week.resets_at)+quotaModule('其他余额',extra,extra,0)+'</div><div class="quota-proxy">代理 '+esc(q.proxy||'直连')+'</div></div>'}).join(''):'<div class="muted small">暂无额度数据</div>'}
