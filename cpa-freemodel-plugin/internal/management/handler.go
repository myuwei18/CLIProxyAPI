...
<style>
:root{color-scheme:light dark;--bg:#f6f7fb;--panel:#fff;--panel2:#f8fafc;--text:#172033;--muted:#667085;--line:#e4e7ec;--blue:#2563eb;--green:#16a34a;--amber:#d97706;--red:#dc2626;--track:#e5e7eb;--shadow:none}
...
function bar(title,used,limit,available){const total=Number(limit||0);const free=available===undefined?Math.max(0,total-Number(used||0)):Number(available||0);const percent=total>0?Math.max(0,Math.min(100,Math.round(free*100/total))):0;const cls=percent>=50?'ok':(percent>=20?'warn':'err');return '<div class="bar-row"><div class="bar-title">'+title+'</div><div><div class="bar"><div class="fill '+cls+'" style="width:'+percent+'%"></div></div></div><div class="bar-meta">'+percent+'%</div></div>'}
...
function renderQuota(resp){latestSummary=resp.summary||{};latestQuota=resp.data||[];updateTabs(latestSummary);document.getElementById('realAvailable').textContent=money(latestSummary.real_available_cents);document.getElementById('quotaHint').textContent='窗口 '+money(latestSummary.window_available_cents)+' · 额外 '+money(latestSummary.extra_available_cents);document.getElementById('otherCount').textContent=text(latestSummary.other);document.getElementById('updatedAt').textContent=fmtTime(new Date().toISOString());const rows=latestQuota.filter(q=>!q.is_test_account);document.getElementById('quotaCards').innerHTML=rows.length?rows.map(q=>{const extra=q.extra_available_cents||0;const st=quotaState(q);const unavailable=st.cls==='ok'?'':' unavailable';return '<div class="quota-card'+unavailable+'"><div class="quota-head"><div><div class="quota-email">'+esc(q.email)+'</div><div class="quota-meta">#'+esc(q.account_id)+' · 有效至 '+fmtTime(q.current_period_end)+'</div><div class="quota-meta">同步 '+fmtTime(q.fetched_at)+'</div></div><div class="quota-status '+st.cls+'"><span class="status-dot '+st.cls+'"></span>'+st.label+'</div></div><div class="quota-main"><div class="quota-num"><div class="name">真实可用</div><div class="amount">'+money(q.real_available_cents)+'</div></div><div class="quota-num"><div class="name">额外可用</div><div class="amount">'+money(extra)+'</div></div></div>'+bar('5h',q.window_5h&&q.window_5h.used_cents,q.window_5h&&q.window_5h.limit_cents,q.window_5h_remaining)+bar('7d',q.window_week&&q.window_week.used_cents,q.window_week&&q.window_week.limit_cents,q.window_week_remaining)+bar('额外',extra>0?0:1,extra>0?extra:1,extra)+'</div>'}).join(''):'<div class="muted small">暂无额度数据</div>'}
