...
function renderQuota(resp){latestSummary=resp.summary||{};latestQuota=resp.data||[];updateTabs(latestSummary);document.getElementById('realAvailable').textContent=money(latestSummary.real_available_cents);document.getElementById('quotaHint').textContent='窗口 '+money(latestSummary.window_available_cents)+' · 额外 '+money(latestSummary.extra_available_cents);document.getElementById('otherCount').textContent=text(latestSummary.other);document.getElementById('updatedAt').textContent=fmtTime(new Date().toISOString());const rows=latestQuota.filter(q=>!q.is_test_account);document.getElementById('quotaCards').innerHTML=rows.length?rows.map(q=>{const st=quotaState(q);const unavailable=st.cls==='ok'?'':' unavailable';return '<div class="quota-card'+unavailable+'"><div class="quota-head"><div style="min-width:0;flex:1"><div class="quota-email">#'+esc(q.account_id)+' '+esc(q.email)+'</div></div><div class="quota-status '+st.cls+'"><span class="status-dot '+st.cls+'"></span>'+st.label+' · '+money(q.real_available_cents)+'</div></div><div class="quota-times"><div class="quota-meta">有效期 '+fmtTime(q.current_period_end)+'</div><div class="quota-meta quota-time-right">更新时间 '+fmtTime(q.fetched_at)+'</div></div><div class="quota-main">'+quotaModule('5h',q.window_5h_remaining,q.window_5h&&q.window_5h.limit_cents,q.window_5h&&q.window_5h.resets_at)+quotaModule('7d',q.window_week_remaining,q.window_week&&q.window_week.limit_cents,q.window_week&&q.window_week.resets_at)+'</div><div class="quota-proxy">代理 '+esc(q.proxy||'直连')+'</div></div>'}).join(''):'<div class="muted small">暂无额度数据</div>'}
